...
// Full-screen framebuffer: 160 x 128 pixels, RGB565 (lv_color_t in default LVGL v8 config)
static lv_color_t framebuffer[SCREEN_WIDTH * SCREEN_HEIGHT];

// LVGL draw buffers (half-screen, matching production setup in lvgl_core.cpp)
// Defined as the globals declared in lvgl_core.h so the real header can be
// reused without pulling in lvgl_core.cpp (which is hardware-specific).
...
// Defined in sp140.ino on hardware; stubbed here
SemaphoreHandle_t lvglMutex = nullptr;

// Registered display handle (returned by lv_disp_drv_register)
static lv_disp_t* main_disp = nullptr;
static bool lvgl_initialized = false;

uint16_t* emulator_get_framebuffer() {
  return reinterpret_cast<uint16_t*>(framebuffer);
}

// Flush callback: copies rendered pixels into our framebuffer
static void emulator_flush_cb(lv_disp_drv_t* disp, const lv_area_t* area, lv_color_t* color_p) {
  ...
  for (int32_t y = 0; y < h; y++) {
    for (int32_t x = 0; x < w; x++) {
      ...
      if (fb_x >= 0 && fb_x < SCREEN_WIDTH && fb_y >= 0 && fb_y < SCREEN_HEIGHT) {
        framebuffer[fb_y * SCREEN_WIDTH + fb_x] = color_p[y * w + x];
      }
    }
  }
...
lv_disp_t* emulator_init_display(bool darkMode) {
  ...
  if (!lvgl_initialized) {
    ...
    disp_drv.flush_cb = emulator_flush_cb;
  ...
  lv_theme_t* theme = lv_theme_default_init(
    ...
    darkMode,
    LV_FONT_DEFAULT);
  lv_disp_set_theme(main_disp, theme);
...
// Helper to convert a framebuffer lv_color_t to 24-bit BGR
static inline void color_to_bgr24(lv_color_t c, uint8_t out[3]) {
  uint16_t rgb565 = c.full;
...
bool emulator_save_bmp(const char* filename) {
  ...
  for (int y = height - 1; y >= 0; y--) {
    for (int x = 0; x < width; x++) {
      uint8_t bgr[3];
      color_to_bgr24(framebuffer[y * width + x], bgr);
      fwrite(bgr, 1, 3, f);
    }
    if (padding > 0) {
...
void emulator_teardown() {
  ...
  if (main_screen != NULL) {
    // Load a throwaway screen first so LVGL's active-screen pointer is not
    // left dangling after we delete main_screen. Without this, the next
    // lv_scr_load() dereferences a freed pointer during transition setup.
